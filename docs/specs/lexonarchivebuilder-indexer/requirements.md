...
- **UR-339 [KNOWN]:** Hitting `Ctrl-C` while an indexer command is using direct
  `local-redb` can leave the redb file marked dirty, which then triggers a very
  large startup recovery delay on the next open.
- **UR-340 [KNOWN]:** LexonArchiveBuilder should install a `Ctrl-C` handler and
  shut down more gracefully so direct `local-redb` stores can close cleanly
  instead of paying that dirty-recovery cost after ordinary operator
  cancellation.
- **UR-341 [KNOWN]:** This graceful-cancel requirement applies to any
  `lexonarchivebuilder-indexer` CLI tool that opens direct `local-redb` and can
  otherwise trigger the same dirty-close behavior.
- **UR-342 [INFERRED]:** The graceful-cancel behavior should remain within the
  existing short-lived CLI process model rather than introducing a daemon,
  separate control plane, or MCP-visible cancellation surface.
- **UR-343 [INFERRED]:** A `Ctrl-C`-triggered graceful shutdown must still end
  as an explicit interrupted or unsuccessful operator outcome rather than a
  success-shaped result for abandoned work.
- **UR-344 [INFERRED]:** Graceful interrupt handling must preserve the existing
...
- **UR-349 [KNOWN]:** The new increment should also pull the latest LexonGraph
  into this change.
- **UR-350 [KNOWN]:** The repository currently pins workspace LexonGraph crates
  to commit `6148d2cc01c2a1017178489bec75aea567297212`.
- **UR-351 [KNOWN]:** The current upstream LexonGraph `main` head resolves to
  `3fa2220cccf601d83f181564aed01cbf40ffcbea`.
- **UR-352 [INFERRED]:** Updating to the latest upstream `main` head is
  acceptable even if unrelated delegated API adjustments are required, so long
  as approved indexer behavior and boundaries are preserved.
- **UR-353 [KNOWN]:** The upstream streaming-indexer crate now supports cancel
  notification, and LexonArchiveBuilder should wire that up for the streaming
  step as part of this increment.
- **UR-354 [INFERRED]:** The graceful-cancel path should propagate the
  repository-owned first-interrupt signal into the delegated streaming-indexer
  execution path rather than waiting only for repository-owned pre/post-
  streaming phases to poll for interrupt.
- **UR-355 [INFERRED]:** While delegated streaming work is active, first
  `Ctrl-C` should still lead to an explicit interrupted or unsuccessful
  operator-visible outcome rather than a success-shaped result.
- **UR-356 [INFERRED]:** Wiring the upstream cancel-notification surface must
  preserve existing CLI-only direct-`local-redb` boundaries and must not
  introduce an MCP-visible cancellation API or daemon-style control plane.
...
- **Streaming cancellation capability [KNOWN]:** When the adopted upstream
  ...
  `f9818730c3f4d02290a617ec6fd7e9f8829eaafd`, that capability becomes part of
  the repository-required latest-main adaptation surface for direct
  `local-redb` streaming execution rather than an optional follow-on
  enhancement.
...
- **Traceability:** UR-47, UR-61, UR-63, UR-64, UR-65, UR-66, UR-67, UR-68, UR-69, UR-71, UR-126, UR-127, UR-128, UR-129, UR-130, UR-140, UR-143, UR-144, UR-145, UR-147, UR-148, UR-220, UR-221, UR-223, UR-224, UR-225, UR-226, UR-227, UR-228, UR-230, UR-231, UR-232, UR-233, UR-244, UR-245, UR-246, UR-247, UR-249, UR-349, UR-350, UR-351, UR-352, UR-353, UR-357
...
#### RQ-INDEXER-005E - Graceful interrupt shutdown for direct local redb

LexonArchiveBuilder SHALL provide repository-owned operator-interrupt handling
for any `lexonarchivebuilder-indexer` CLI command that opens a direct
`local-redb` block store.

- **Trigger [KNOWN]:** On operator `Ctrl-C`, a command that currently holds a
  direct `local-redb` store open SHALL begin graceful shutdown instead of
  exiting abruptly at the first interrupt boundary it controls.
- **Shutdown objective [KNOWN]:** That graceful shutdown SHALL stop accepting
  new repository-owned work quickly enough to allow repository-owned direct
  local redb handles to close cleanly, so ordinary operator cancellation does
  not leave the store in the known dirty-close state that triggers long startup
  recovery on the next open.
- **Applicability [KNOWN]:** This requirement applies across batch `run`,
  rooted quality, rooted search, rooted copy, `maintenance`, and future
  indexer CLI surfaces whenever they open direct `local-redb` and can
  otherwise trigger the same dirty-close behavior.
- **Outcome boundary [INFERRED]:** A `Ctrl-C`-triggered graceful shutdown SHALL
  still end as an explicit interrupted or otherwise unsuccessful operator
  outcome; it SHALL NOT emit a success-shaped summary, report, or maintenance
  result for abandoned work.
- **Mutable-ref boundary [INFERRED]:** Graceful interrupt handling SHALL
  preserve the existing rule that repository-owned mutable refs remain outside
  redb on the filesystem and SHALL NOT treat unpublished or incomplete work as
  authoritative merely because shutdown was orderly.
- **Surface boundary [INFERRED]:** This behavior remains a short-lived CLI
  process concern and SHALL NOT introduce a new daemon, background service, or
  MCP-visible cancellation surface.
- **Environment boundary [KNOWN]:** This requirement is specific to direct
...
- **Bounded delay [INFERRED]:** Repository-owned interrupt handling SHALL NOT
  ...
  code.
- **Delegated streaming coverage [KNOWN]:** When a direct `local-redb` command
  is actively executing delegated work through an upstream
  `lexongraph-streaming-indexer` path that exposes cancel notification,
  currently the constrained v3 path added by commit
  `f9818730c3f4d02290a617ec6fd7e9f8829eaafd`, LexonArchiveBuilder SHALL
  propagate the repository-owned first-interrupt signal through that upstream
  cancel-notification surface.
- **Delegated timing [INFERRED]:** LexonArchiveBuilder SHALL NOT rely solely on
  repository-owned polling before or after the delegated streaming step if the
  active upstream streaming phase can continue for operator-significant time
  after first `Ctrl-C`.
- **Outcome preservation [KNOWN]:** Propagating delegated cancel notification
  SHALL preserve the existing rule that interrupted work ends as an explicit
  interrupted or otherwise unsuccessful operator outcome rather than a
  success-shaped summary or published result.
- **Traceability:** UR-339, UR-340, UR-341, UR-342, UR-343, UR-344, UR-345,
  UR-346, UR-347, UR-348, UR-353, UR-354, UR-355, UR-356, UR-357
...
- **Before [KNOWN]:** Indexer CLI surfaces that open direct `local-redb` did not
  yet have a repository-owned graceful `Ctrl-C` shutdown requirement, so
  ordinary operator cancellation could leave the store dirty and force a long
  recovery delay on the next open.
- **After [KNOWN]:** The requirements now mandate graceful `Ctrl-C` handling for
...
- **Before [KNOWN]:** The workspace pinned LexonGraph crates to
  `6148d2cc01c2a1017178489bec75aea567297212`, so this repository did not yet
  adopt upstream `main` head `3fa2220cccf601d83f181564aed01cbf40ffcbea`.
- **After [KNOWN]:** The requirements now authorize refreshing the shared
  LexonGraph dependency set to upstream `main` head
  `3fa2220cccf601d83f181564aed01cbf40ffcbea` while preserving approved
  stage, search-serving, and mutable-ref boundaries.

### BA-INDEXER-151

- **Before [KNOWN]:** The graceful interrupt requirements covered
  repository-owned phases but did not yet require LexonArchiveBuilder to use
  the newly available delegated constrained streaming-indexer v3
  cancel-notification surface while upstream streaming work was active.
- **After [KNOWN]:** The requirements now require direct-`local-redb`
  constrained-v3 streaming execution to propagate first interrupt through the
...
- **UR-REFRESH-008 [KNOWN]:** Pick up LexonGraph commit
  `7ce519b9fa267065b9b0acfcfc2cb180db5abaaf`.
- **UR-REFRESH-009 [KNOWN]:** Update the Cargo manifests and lockfile, plus
  active LexonGraph commit references in the repository specifications, to
  reflect the requested baseline.
- **UR-REFRESH-010 [INFERRED]:** Preserve existing LexonArchiveBuilder
  indexing, MCP, storage, embedding, telemetry, cancellation, idempotence,
  recoverability, and content-type extensibility contracts unless compatibility
  analysis proves a concrete upstream incompatibility.

### Change Manifest

| ID | Type | Summary | Traceability |
|---|---|---|---|
| CM-REFRESH-008 | Revise | Adopt LexonGraph commit `7ce519b9fa267065b9b0acfcfc2cb180db5abaaf` as the active immutable dependency baseline | UR-REFRESH-008 |
| CM-REFRESH-009 | Revise | Align Cargo manifests, lockfile resolution, and active specification references with the requested baseline while retaining prior revisions as historical evidence | UR-REFRESH-009 |
| CM-REFRESH-010 | Preserve | Retain existing repository-visible behavior and architectural boundaries unless the refreshed revision introduces a demonstrated incompatibility | UR-REFRESH-010 |

### Before / After

#### BA-REFRESH-005

- **Before [KNOWN]:** The active workspace baseline is LexonGraph commit
  `caa5249e641d1361998bb2410e79893579262bc5`; prior revisions remain
  historical references.
- **After [KNOWN]:** The active workspace baseline for this increment is
  LexonGraph commit `7ce519b9fa267065b9b0acfcfc2cb180db5abaaf`; the prior
  baseline remains a historical reference.

#### BA-REFRESH-006

- **Before [KNOWN]:** Cargo resolution artifacts and active specification
  references identify the previous LexonGraph baseline.
- **After [INFERRED]:** Cargo manifests, lockfile entries, and active
  specification references identify the requested baseline consistently, while
  historical revision references remain traceable and are not rewritten as
  active targets.

### Requirements

#### RQ-REFRESH-008 - Coordinated active upstream baseline

The LexonArchiveBuilder workspace SHALL use LexonGraph commit
`7ce519b9fa267065b9b0acfcfc2cb180db5abaaf` as the active immutable revision
for all currently declared LexonGraph packages, and all such packages SHALL
resolve to that same revision.

- **Traceability:** UR-REFRESH-008, UR-REFRESH-009

#### RQ-REFRESH-009 - Specification and Cargo alignment

The Cargo manifests, lockfile, and active repository specification references
SHALL identify the requested LexonGraph revision consistently. Earlier
LexonGraph revisions SHALL remain only as historical evidence unless they are
also independently active targets.

- **Traceability:** UR-REFRESH-009

#### RQ-REFRESH-010 - Compatibility-preserving refresh

The refresh SHALL preserve existing caller-visible indexing and MCP behavior,
storage and embedding boundaries, status and liveness projections,
cancellation, immutable-block idempotence, replay recoverability, and future
content-type extensibility unless the requested revision makes a contract
impossible to preserve.

- **Traceability:** UR-REFRESH-010

#### RQ-REFRESH-011 - Explicit incompatibility handling

If the requested LexonGraph revision changes or removes a repository-required
API or behavior, the incompatibility SHALL be recorded explicitly in the
downstream design, implementation, and validation artifacts. Required behavior
SHALL NOT be silently dropped or replaced with an unapproved repository-local
substitute.
...
- **MCP contract:** Preserved [KNOWN].
- **Storage and embedding adapters:** Preserved [KNOWN].
- **External stage, telemetry, and cancellation contracts:** Require
  compatibility verification [INFERRED].
- **Idempotence, recoverability, and content-type extensibility:** Preserved
  unless the upstream revision changes those contracts [INFERRED].
- **Cargo/specification alignment:** Changed [KNOWN] by the requested revision
  refresh.
- **Implementation changes:** [UNKNOWN] until the requested revision is
